...
* @version $Id: functions.php, v 1.001 2012/11/14 Martin Truckenbrodt Exp$
...
function check_httpbl($action = 'recheck', $ip = false, $username = false, $email = false, $message = false)
{
	...
	if (($ip || $username || $email || $message))
	{
		...
		while ($row = $db->sql_fetchrow($result))
		{
			...
			// a key always is needed
			else if ($row['httpbl_name'] == 'honeypot' && $row['httpbl_key'] && $ip)
			{
				$apiurl = '';
...
function check_spam($mode = false, $action = 'recheck', $ip = false, $user_id = 0, $post_id = 0, $data = false, $message = false)
{
	...
	// IPRBL check
	if ($user_data['user_ip'] && (((($config['check_iprbl_post'] == SPAM_CHECK_ALL || ($config['check_iprbl_post'] == SPAM_CHECK_GUESTS && $user_data['user_id'] == ANONYMOUS)) && $mode == 'post') || ($config['check_iprbl_register'] && $mode == 'user' && $action == 'register')) || $action == 'recheck'))
	{
	...
	if ($user_data['user_email'] && ((($config['check_domainrbl_email'] == SPAM_CHECK_ALL || $config['check_domainrbl_email'] == SPAM_CHECK_GUESTS) && ($config['break_after_httpbl'] && $info) == false) || $action == 'recheck'))
	{
