		...
		switch ($mode)
		{
			case 'overview':
					...
					if ($delete && $user_row['user_type'] != USER_FOUNDER)
					{
						...
						if ($delete_type)
						{
							if (confirm_box(true))
							{
								...
								trigger_error($user->lang['USER_DELETED'] . (($delete_type == 'report_spammer') ? '<br />' . $user->lang['USER_REPORTED'] : '') . adm_back_link($this->u_action));
							}
							else
							{
				...
				$template->assign_vars(array(
					...
					'U_RECHECK_SPAM'	=> $this->u_action . "&amp;action=recheck_spam&amp;user_ip={$user_row['user_ip']}&amp;user_id={$user_row['user_id']}",
