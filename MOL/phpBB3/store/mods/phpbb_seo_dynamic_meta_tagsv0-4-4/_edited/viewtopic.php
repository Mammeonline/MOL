...
// BEGIN Print all posts in topic mod
if ($view == 'printall')
{
   $start = '0';
...
// following we add an else to the if statement
elseif ($start < 0 || $start >= $total_posts)
{
...
'GALLERY_IMG'		=> $user->img('icon_contact_gallery', 'PERSONAL_ALBUM'),
	...
	// www.phpBB-SEO.com SEO TOOLKIT END
// BEGIN Print all posts in topic mod
       'U_PRINT_ALL_POSTS'     => ($auth->acl_get('f_print', $forum_id)) ? append_sid("{$phpbb_root_path}viewtopic.$phpEx", "f=$forum_id&amp;t=$topic_id&amp;start=$start" . ((strlen($u_sort_param)) ? "&amp;$u_sort_param" : '') . '&amp;view=printall' ) : '',
// END Print all posts in topic mod
...
for ($i = 0, $end = sizeof($post_list); $i < $end; ++$i)
{
	...
	$postrow = array(
		...
		'U_REPORT_SPAM'		=> ($auth->acl_get('a_user')) ? append_sid("{$phpbb_root_path}posting.$phpEx", "mode=report_spam&amp;f=$forum_id&amp;p={$row['post_id']}") : '',
...
$template->set_filenames(array(
	'body' => ($view == 'print' || $view == 'printall') ? 'viewtopic_print.html' : 'viewtopic_body.html')
);
